<template>
  <div class="user-box" v-if="authState.user" @click="open = !open" v-click-outside="close">
    <div class="avatar" :style="{ background: `linear-gradient(135deg, ${c[0]}, ${c[1]})` }">
      {{ avatarText(authState.user.nickname) }}
    </div>
    ...
    <transition name="pop">
      <div class="panel" v-if="open" @click.stop>
        <div class="panel-head">
          ...
          <div class="info">
            <div class="p-nick">{{ authState.user.nickname }}</div>
            <div class="p-account">@{{ authState.user.username }}</div>
          </div>
        </div>
        <div class="panel-body">
          <div class="row"><span class="k">账号</span><span class="v">{{ authState.user.username }}</span></div>
          <div class="row"><span class="k">昵称</span><span class="v">{{ authState.user.nickname }}</span></div>
          <div class="row" v-if="authState.user.created_at">
            <span class="k">注册时间</span><span class="v">{{ authState.user.created_at.slice(0, 10) }}</span>
          </div>
        </div>
...
import { authState, clearAuth, avatarColors, avatarText } from '../auth'

const router = useRouter()
const open = ref(false)
const c = avatarColors(authState.user?.username || '')
...
.nick {
  font-size: 14px; color: var(--text); max-width: 100px;
