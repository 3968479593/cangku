...
class libraryManager:
  ...
  # 删除图书
    def delete_book(self,book_id):
        try:
            self.cursor.execute("SELECT name FROM book WHERE book_id=%s",(book_id,))
            book = self.cursor.fetchone()
            if not book:
                print("未找到该图书，删除失败")
                return
                
            confirm = input(f"确认要删除图书{book['name']} (编号:{book_id})吗？(y/n)：")
            if confirm.lower() != "y":
                print("已取消删除操作")
                return
            ...
            self.conn.commit()
            print("删除成功")
            self.write_log(f"管理员删除图书：{book_id}《{book['name']}》")
        except Exception as e:
            self.conn.rollback()
            print("删除失败：", e)
   # 借阅
    def borrow_book(self, book_id, user_id, role):
        try:
            self.cursor.execute("SELECT zhuangtai, name FROM book WHERE book_id=%s", (book_id,))
            book = self.cursor.fetchone()
            if not book:
                print("无此书")
                return
            ...
            return_date = (now + datetime.timedelta(days=7)).strftime("%Y-%m-%d")
            ...
            self.write_log(f"用户：{user_id} 身份：{role} 借出图书：《{book['name']}》(ID:{book_id}) 借阅日期：{borrow_date} 归还日期：{return_date}")
    ...
    def return_book(self, book_id, user_id, role):
        try:
            ...
            print("归还成功")
            self.write_log(f"用户[{user_id}] ({role}) 归还图书：《{book['name']}》(ID:{book_id})")
